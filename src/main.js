...
async function step7_launcher(claudePath, tvPath, mcpDir) {
  ...
  const script = [
    ...
    `WRAPPER="${wrapperPath}"`,
    '',
    '# Trova Claude Code dinamicamente ad ogni avvio (evita path hardcoded che cambia)',
    'CLAUDE=$(which claude 2>/dev/null)',
    'if [ -z "$CLAUDE" ]; then',
    '  for p in /opt/homebrew/bin/claude /usr/local/bin/claude "$HOME/.npm-global/bin/claude" "$HOME/Library/Application Support/npm/bin/claude"; do',
