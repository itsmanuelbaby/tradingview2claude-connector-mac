...
async function step7_launcher(claudePath, tvPath, mcpDir) {
  ...
  if (IS_WIN) {
    ...
    const lines = [
      ...
      '',
      ':: Aspetta che CDP sia effettivamente pronto (max 30 secondi)',
      'echo  Attendo che TradingView sia pronto...',
      'set CDP_READY=0',
      'set RETRY=0',
      ':WAIT_CDP',
      'timeout /t 2 /nobreak >nul',
      'curl -s http://localhost:9222/json/version >nul 2>&1 && set CDP_READY=1',
      'if "%CDP_READY%"=="1" goto CDP_OK',
      'set /a RETRY+=1',
      'if %RETRY% lss 15 goto WAIT_CDP',
      'echo  ATTENZIONE: CDP non risponde, procedo comunque...',
      ':CDP_OK',
      'echo  [OK] TradingView pronto',
      '',
