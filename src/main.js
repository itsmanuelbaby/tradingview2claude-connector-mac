...
async function step7_launcher(claudePath, tvPath, mcpDir) {
  ...
  const script = [
    ...
    '',
    '# Trova Claude Code — cerca prima nell\'app ufficiale (path con versione), poi npm',
    'CLAUDE=""',
    'CLAUDE_APP_DIR="$HOME/Library/Application Support/Claude"',
    'if [ -d "$CLAUDE_APP_DIR" ]; then',
    '  CLAUDE=$(find "$CLAUDE_APP_DIR" -name "claude" -type f 2>/dev/null | grep -v ".app/Contents" | sort -V | tail -1)',
    '  if [ -z "$CLAUDE" ] || [ ! -x "$CLAUDE" ]; then',
    '    CLAUDE=$(find "$CLAUDE_APP_DIR" -name "claude" -type f 2>/dev/null | sort -V | tail -1)',
    '  fi',
    'fi',
    'if [ -z "$CLAUDE" ] || [ ! -x "$CLAUDE" ]; then',
    '  for p in \\',
